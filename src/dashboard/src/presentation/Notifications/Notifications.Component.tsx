import { useEffect, useReducer, useState, type FormEvent, type ChangeEvent } from 'react'
...
import { buildNotificationsPresenter } from './Notifications.Presenter'
import { notificationsReducer } from './Notifications.Reducer'
import { buildInitialNotificationsUido, type ChannelId } from './Notifications.Uido'
...
export function NotificationSettingsView() {
  ...
  const [uido, dispatch] = useReducer(
    ...
    undefined,
    buildInitialNotificationsUido,
  )
  const presenter = usePresenter(buildNotificationsPresenter, { container, dispatch })

  const notificationSettings = useNotificationSettings(
    uido.selectedChannel,
    container.getNotificationChannelConfig,
    container.saveNotificationChannelConfig,
    container.testNotificationChannel,
    container.deleteNotificationChannel,
  )
  ...
  useEffect(() => {
    presenter.onRefreshLog(uido.selectedChannel)
  }, [presenter, uido.selectedChannel])
    ...
    <div className="app-shell app-shell-cameras">
      ...
      <div className="camera-master-detail">
          ...
          <div className="camera-sidebar-group">
            ...
            {CHANNELS.map((channel) => (
              <button
                key={channel.id}
                type="button"
                className={`camera-nav-item${uido.selectedChannel === channel.id ? ' selected' : ''}`}
                onClick={() => presenter.onSelectChannel(channel.id)}
              >
                ...
                <div className="camera-nav-meta">
                  {channel.id === uido.selectedChannel && (
                    <ChannelStatusBadge
                      config={notificationSettings.config}
                      loading={notificationSettings.loading}
                    />
        ...
        <div className="camera-detail-panel panel">
          {uido.selectedChannel === 'telegram' && (
            <TelegramConfigPanel
              notifLog={uido.notifLog}
              detectionLabels={uido.detectionLabels}
              onRefreshLog={() => presenter.onRefreshLog(uido.selectedChannel)}
...
function MessageFieldsSelector({
  ...
  function toggle(field: string) {
    if (selected.includes(field)) {
      ...
      onChange(next.length > 0 ? next : [field])
    } else {
...
function TelegramConfigPanel({
  ...
  detectionLabels,
  onRefreshLog,
  settings: { config, loading, saving, testing, removing, testResult, save, test, remove },
}: {
  ...
  async function handleSaveWithRefresh(req: SaveNotificationChannelConfigRequest) {
    await save(req)
    onRefreshLog()
  }
  ...
  async function handleSave(e: FormEvent) {
    ...
    await handleSaveWithRefresh(req)
    setBotToken('')
  }

  async function handleConfirmSave() {
    if (!pendingSaveReq) return
    await handleSaveWithRefresh(pendingSaveReq)
