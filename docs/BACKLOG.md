...
- **Refondre le socle de configuration** (navigation, architecture de l'information, edition). Chantier a cadrer avant tout code : c'est une decision d'architecture de l'information, pas un deplacement de boutons. A mener avec l'item bibliotheque de composants ci-dessous, dont il partage le declencheur.

  **Objectif** : poser une base saine et evolutive, car le nombre de parametres va croitre fortement. Tenir le paradoxe central du produit — une configuration **extremement simple pour un utilisateur non-technicien** (principe #1), sans renoncer aux **reglages de niche** qui couvrent les besoins reels et evitent la friction d'un systeme dont on ne sait pas ce qu'il fait (principe #4). La reponse classique est la divulgation progressive (paliers simple / avance, sections repliees) ; a trancher.

  1. **Ou se configure quoi n'a aucune coherence.** Symptome le plus net, constate en livrant [ADR-39](adr/0039-reglages-globaux-surchargeables-par-camera-retention-d-enregistrement.md) : les reglages valables pour toute l'installation ont ete poses dans une entree de la barre laterale de la page Cameras, au milieu de la liste des cameras decouvertes — faute d'endroit correct ou les mettre. La page a ete renommee « Parametres » pour limiter les degats, ce qui ne fait que deplacer l'incoherence (une page « Parametres » dont la fonction principale est d'ajouter des cameras). A questionner aussi : « Alertes » est en realite un ecran de reglages, « Expert » n'a pas de perimetre defini, et rien ne distingue *configurer une camera*, *configurer l'installation* et *consulter*.

  2. **Rassembler les parametres en un seul endroit** : notifications, alertes, et reglages systeme (modele d'inference, seuil d'alerte stockage, seuil d'alerte CPU a venir) rejoignent les cameras dans une meme arborescence, plutot que d'etre disperses par accident d'historique.

  3. **Mobile first, aujourd'hui absent alors que c'est la cible.** L'interface est concue au clavier-souris et adaptee ensuite. A noter : SPECS §7.2 n'exige aujourd'hui que des actions « faisables sur mobile et desktop », ce qui est plus faible qu'une conception mobile-first — les SPECS sont donc a mettre a jour avec la direction retenue.

  4. **Le cycle enregistrer / appliquer / annuler n'est ni clair ni uniforme.** Trois etats se superposent sans vocabulaire commun : la modification locale, l'enregistrement cote Vyzio, et l'application au moteur de detection (bandeau « configuration a appliquer », ADR-38). L'utilisateur ne voit pas **ce qui a ete modifie** avant d'enregistrer, et ne peut pas annuler. Le bouton d'enregistrement apparait au fil de l'ecran, souvent hors de la zone visible sur une interface deja chargee : l'appel a l'action passe inapercu. La retention livree avec ADR-39 ajoute encore une variante (enregistrement automatique a la sortie du champ, sans bouton) — coherent avec la fiche camera, en rupture avec le reste. A uniformiser, pas a rustiner.

  5. **Trop de texte explicatif.** Les explications sont utiles mais noient les noms et les valeurs des reglages, avec lesquels elles se confondent. Viser une **vue compacte**, rangee par categorie (le plus courant en premier, puis une section par fonctionnalite en sous-menu), et basculer l'aide en **info-bulle ou equivalent** — disponible sans occuper la place. Attention au piege mobile : une info-bulle au survol est inatteignable au doigt, il faut un declencheur explicite.

  6. **Le desordre est aussi structurel cote code.** L'ecran Cameras melange decouverte, onboarding, fiche detaillee et reglages generaux dans un seul composant de plus de mille lignes, avec une union `CameraSelection` qui sert a la fois de selection d'objet et de routage d'ecran — c'est precisement ce qui a rendu l'ajout des reglages generaux si facile, et donc ce qui masque le probleme.
- Revoir la gestion des composants d'interface, avec une bibliotheque type shadcn/ui plutot que des composants maison sur CSS global. Declencheur : les `select` etaient blancs sur blanc (fond translucide sur carte blanche), corrige au cas par cas alors que le probleme est systemique — pas de tokens de contraste, pas d'etats focus/disabled coherents, pas d'accessibilite garantie. A confronter au [DESIGN SYSTEM](DESIGN%20SYSTEM.md) existant : l'enjeu est de savoir si on adopte une base de composants ou si on outille davantage la notre.
